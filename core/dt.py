...
"""
create_bus_nodes: append the peripherals to its own bus

This function create a data structure to append the peripherals
...
def create_bus_nodes(soc_config):
    ...
    for bus in bus_cfg:
        bus_name = bus_cfg[bus]['name']
        bus_label = bus_cfg[bus]['label']
        ...
        buses_node[bus].update(bus_cfg[bus])
        peri_config = soc_config['peripherals']

        for peripheral in available_peripherals:
            for pp in peri_config[peripheral]:
                peri_name = peri_config[peripheral][pp]['name']
                peri_label = peri_config[peripheral][pp]['label']
